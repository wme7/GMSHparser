...
[V,VE,SE,~,~,~,info] = GMSHparserV2('../meshes/extruded_prism_v2.msh');
...
[V,VE,SE,~,~,~,info] = GMSHparserV2('../meshes/extruded_hex_v2.msh');
...
[V,VE,SE,~,~,~,info] = GMSHparserV2('../meshes/extruded_mixed_v2.msh');
...
[V,VE,SE,~,~,~,info] = GMSHparserV4('../meshes/extruded_prism_v4.msh');
...
[V,VE,SE,~,~,~,info] = GMSHparserV4('../meshes/extruded_hex_v4.msh');
...
[V,VE,SE,~,~,~,info] = GMSHparserV4('../meshes/extruded_mixed_v4.msh');
...
function viewLineElements(V,E,info)
    ...
    x = V(:,1); y = V(:,2); z = V(:,3);
    lx=x(E.EToV); ly=y(E.EToV); lz=z(E.EToV);
    xc=mean(lx,2); yc=mean(ly,2); zc=mean(lz,3);
    hold on
    for i=1:length(E.EToV)
